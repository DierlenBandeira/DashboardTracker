...
CSS_PATH = BASE_DIR / "dashboard.css"
JS_PATH = BASE_DIR / "dashboard.js"
...
def load_dashboard_html() -> str:
    ...
    <body>
      <h2>dashboard.html não encontrado</h2>
      <p>Coloque o arquivo dashboard.html na mesma pasta do dashboard_collector.py.</p>
    </body>
    </html>
    """
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_file(self, file_path: Path, content_type: str, not_found_msg: str):
        try:
    ...
    def do_GET(self):
        ...
        if path == "/dashboard.css":
            self._serve_file(CSS_PATH, "text/css; charset=utf-8", "dashboard.css nao encontrado")
            return

        if path == "/dashboard.js":
            self._serve_file(JS_PATH, "application/javascript; charset=utf-8", "dashboard.js nao encontrado")
            return

        if path == "/logo.png":
            self._serve_file(LOGO_PATH, "image/png", "logo.png nao encontrada")
            return
...
def main():
    ...
    print(f"[PY] JS esperado: {JS_PATH}", flush=True)
    print(f"[PY] Logo esperada: {LOGO_PATH}", flush=True)
    print(f"[PY] Host: {HTTP_HOST}:{HTTP_PORT}", flush=True)
    print(f"[PY] CORS origin: {CORS_ALLOW_ORIGIN}", flush=True)

    t = threading.Thread(target=poll_loop, daemon=True)
    t.start()

    server = ThreadingHTTPServer((HTTP_HOST, HTTP_PORT), Handler)
    print(f"OK: servidor em http://{HTTP_HOST}:{HTTP_PORT}/", flush=True)
